...
<?php
    ...
    if (isset($_REQUEST['submit'])) {
        ...
        if (count($errors) == 0) {
        $query    = "INSERT into `users` (name, email, phoneno, password, address, waddress,service,image, usertype, create_datetime)
                     VALUES ('$name', '$email', '$phoneno', '" . md5($password) . "', '$address', '$waddress','$service','$image', '$usertype', '$create_datetime')";
